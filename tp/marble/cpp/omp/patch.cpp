...
void Patch::initTopology(struct Parameters params,
    ...
    if ((patch_x_length <= 2*params.radius || patch_y_length <= 2*params.radius || patch_z_length <= 2*params.radius)) {
        std::cerr << " CONFIGURATION ERROR: a patch must be larger than a particle diameter."
                  << " (radius: " << params.radius << " - length x: " << patch_x_length << ")"
                  << std::endl;
        exit(0);
    }
...
void Patch::initParticles(struct Parameters params) {
    ...
    for (int ip = 0 ; ip < local_number ; ip++) {
        ...
        v     = dis(gen) * (params.vmax - params.vmin) + params.vmin;
...
void Patch::patchIndexToCoordinates(struct Parameters params, int id, int & id_x, int & id_y, int & id_z) {
    ...
    id_y = (id - id_z * (params.n_patches_y*params.n_patches_x)) / params.n_patches_x;
...
// Check that all particles are in the domain
void Patch::print(struct Parameters params) {
    
    std::cout << " Patch "<< id << ": "
    << " - coordinates: " << id_x << " " << id_y << " " << id_z
    << " - xmin: " << xmin << " - xmax: " << xmax
    << " - ymin: " << ymin << " - ymax: " << ymax
    << " - zmin: " << zmin << " - zmax: " << zmax
    << std::endl;
