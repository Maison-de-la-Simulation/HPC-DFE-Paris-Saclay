...
int main( int argc, char *argv[] )
{
    ...
    int particle_number;            // Number of particles in the simulation
    int imbalance;                  // Imbalance
    ...
    int exchange_counter;           // Counter for exchanges per timestep
    ...
    MPI_Comm_size( MPI_COMM_WORLD, &params.number_of_ranks );
    ...
    if (params.rank == 0) system("mkdir -p diags");
    ...
    if (params.rank == 0) {
        ...
        std::cout << "  - ranks in x: " << params.n_patches_x << std::endl;
        std::cout << "  - ranks in y: " << params.n_patches_y << std::endl;
        std::cout << "  - ranks in z: " << params.n_patches_z << std::endl;
        std::cout << "  - exchange method: " << params.exchange << std::endl;
    ...
    for (params.iteration = 1 ; params.iteration <= params.final_iteration; params.iteration++) {
        ...
        //collision_counter = particles.collisions(params, params);
        ...
        // Global information __________________
        ...
        if (params.iteration%params.print_period == 0) {
            ...
            if (params.rank == 0) {
                std::cout << " Iteration: " << std::setw(5) << params.iteration
                          ...
                          << " - exchanges: " << std::setw(3) << exchange_counter
    ...
    // Destruction _________________________________________________________
    
    delete [] params.topology_map;
