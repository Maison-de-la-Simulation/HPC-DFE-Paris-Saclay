...
module timers
    ...
    subroutine timers_show()
        ...
        if (rank.EQ.0) then
            ...
            write(*,'(X,"Total               ",3("|",X,F10.3,X),"|",X,F10.2,"%|")') &
            minimum_timers(2)+minimum_timers(1),average_timers(2)+average_timers(1),&
            maximum_timers(2)+maximum_timers(1),percentages(2)+percentages(1)
